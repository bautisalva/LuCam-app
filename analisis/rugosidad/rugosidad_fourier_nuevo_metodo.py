"""
RUGOSIDAD — FOURIER + FAJAS (script final con logs, overlay ECG y **normales por FFT**)

Cambia respecto de la versión anterior:
  • Las normales en la curva suavizada C^(K) ahora se calculan por **derivada espectral** (FFT),
    en lugar de usar gradiente del EDT de la máscara rasterizada.
  • Se puede alternar el método de normales desde CFG["NORMAL_METHOD"] = "fft" | "edt".
...
CFG = dict(
    ...
    K_FINE_MIN=1,
    K_FINE_MAX=80,
    K_FINE_N=40,               # cantidad de puntos en linspace

    # --- Comparación en eje común (plot 5):
    K_OVERLAY=[1, 2, 3, 20],   # los K a superponer con Pablo

    # --- Método de normales:
    NORMAL_METHOD="fft",       # "fft" (nuevo, recomendado) | "edt" (anterior)
...
# ================================
# NORMALES (dos métodos: FFT o EDT)
# ================================

def normals_via_fft(curve_xy: np.ndarray) -> np.ndarray:
    """Normales unitarias sobre curva cerrada usando derivada espectral (FFT).
    curve_xy: array (M,2) con puntos en orden periódico (no hace falta cerrar manualmente).
    Devuelve N_dir (M,2) con la normal unitaria (rotación +90° de la tangente unitaria).
    """
    z = curve_xy[:,0] + 1j*curve_xy[:,1]
    N = z.size
    # FFT y derivada espectral: d/dt  ↔  i 2π k
    Z = np.fft.fft(z)
    k = np.fft.fftfreq(N) * N  # enteros (…,-2,-1,0,1,2,…)
    dz_dt = np.fft.ifft(1j * 2*np.pi * k * Z)
    # Tangente unitaria
    speed = np.abs(dz_dt) + 1e-12
    T = dz_dt / speed
    # Normal = i * T  (rotación +90°)
    N_complex = 1j * T
    N_dir = np.column_stack([N_complex.real, N_complex.imag]).astype(float)
    return N_dir

# ----- Soporte para método EDT (opcional, como respaldo) -----
...
def normals_dispatch(curve_xy: np.ndarray, method: str, mask_shape=None) -> np.ndarray:
    method = (method or "fft").lower()
    if method == "fft":
        return normals_via_fft(curve_xy)
    elif method == "edt":
        if mask_shape is None:
            raise ValueError("normals_dispatch(method='edt') requiere mask_shape para rasterizar.")
        M = rasterize_closed(curve_xy, mask_shape)
        _, gx, gy = signed_distance_and_grad(M)
        N_dir = np.zeros_like(curve_xy)
        for j, p in enumerate(curve_xy):
            g = np.array([bilinear(gx, p[0], p[1]), bilinear(gy, p[0], p[1])], float)
            nv = np.linalg.norm(g)
            if nv < 1e-10:  # respaldo geométrico local
                j0 = (j-1) % len(curve_xy); j1 = (j+1) % len(curve_xy)
                t = curve_xy[j1] - curve_xy[j0]
                t /= (np.linalg.norm(t) + 1e-12)
                N_dir[j] = np.array([-t[1], t[0]])  # +90°
            else:
                N_dir[j] = g / nv
        return N_dir
    else:
        raise ValueError("NORMAL_METHOD debe ser 'fft' o 'edt'.")
...
def u_B_por_K(mask, Msamples, K, fit_range_log10):
    ...
    # --- NUEVO: normales por FFT o EDT según CFG ---
    if CFG.get("NORMAL_METHOD", "fft").lower() == "fft":
        print(f"[normales] método FFT (derivada espectral) para K={K}…")
        N_dir = normals_via_fft(C_ref)
    else:
        print(f"[normales] método EDT (gradiente de distancia) para K={K}…")
        N_dir = normals_dispatch(C_ref, method="edt", mask_shape=mask.shape)
...
def overlay_curves_common_axis_ecg(mask, Msamples, K_list, N_TH, fit_range_log10):
    ...
    for K in K_list:
        ...
        C_ref = np.column_stack([x_ref, y_ref])

        # --- NUEVO: normales por FFT o EDT ---
        if CFG.get("NORMAL_METHOD", "fft").lower() == "fft":
            N_dir = normals_via_fft(C_ref)
        else:
            N_dir = normals_dispatch(C_ref, method="edt", mask_shape=mask.shape)

        u, _ = fajas_u_Q(C_ref, np.column_stack([xs, ys]), N_dir)
...
if __name__ == "__main__":
    print("[inicio] Script de rugosidad (Fourier + Fajas) — normales:", CFG.get("NORMAL_METHOD","fft").upper())
