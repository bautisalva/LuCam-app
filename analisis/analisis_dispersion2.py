...
# ------------------------------------------------------------
# Clase para mejorar imágenes no binarizadas (sin cambios)
# ------------------------------------------------------------
class ImageEnhancer:
    def __init__(self, imagen, sigma_background=100, alpha=0):
        self.image = imagen
    ...
    def procesar(self, suavizado=3, percentil_contornos=99.9):
        ...
        threshold = threshold_otsu(smooth)
        binary = (smooth > threshold).astype(np.uint16)
        contours = self._find_large_contours(
            binary, percentil_contornos=percentil_contornos
        )
        return binary, contours


# ------------------------------------------------------------
# Cálculo de Var(u) (sin cambios)
# ------------------------------------------------------------
def var_u(binary1, binary2, contours1):
    delta_a = binary2.astype(int) - binary1.astype(int)
    changed_pixels = np.argwhere(delta_a != 0)
...
# ------------------------------------------------------------
# Cargar imágenes desde carpeta (sin cambios)
# ------------------------------------------------------------
def load_images_folder(folder, regex_pattern, min_index=15):
    pattern = re.compile(regex_pattern)
    files = []
...
# ------------------------------------------------------------
# Guardar imagen con contornos superpuestos para debug
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Pipeline principal con logging y controles adicionales
# ------------------------------------------------------------
def pipeline(folder, regex_pattern, already_binary=False):
    images, filenames = load_images_folder(folder, regex_pattern, min_index=15)

    # Preparar carpeta de debug y archivo CSV
    debug_folder = os.path.join(folder, "debug_contornos")
    os.makedirs(debug_folder, exist_ok=True)
    csv_path = os.path.join(folder, "pipeline_log.csv")
    ...
    for k in range(len(images) - 1):
        ...
        # Binarización y extracción de contornos
        if already_binary:
            binary1 = (images[k] > 0).astype(np.uint8)
            binary2 = (images[k + 1] > 0).astype(np.uint8)
            contours1 = find_contours(binary1, level=0.5)
            ...
            enhancer1 = ImageEnhancer(images[k], sigma_background=100, alpha=0)
            enhancer2 = ImageEnhancer(images[k + 1], sigma_background=100, alpha=0)
            binary1, contours1 = enhancer1.procesar(
                suavizado=3, percentil_contornos=99.9
            )
            binary2, _ = enhancer2.procesar(
                suavizado=3, percentil_contornos=99.9
            )

        # Control de consistencia de dimensiones
        if binary1.shape != binary2.shape:
            raise ValueError(
                f"Mismatch de shapes en paso {k}: "
                f"{binary1.shape} vs {binary2.shape}"
            )

        # Guardar debug visual del paso k
        base = f"step_{k:03d}_{filenames[k].rsplit('.',1)[0]}"
        save_contour_overlay(binary1, contours1, debug_folder, base)

        # Métricas intermedias
        delta_a = binary2.astype(int) - binary1.astype(int)
        changed = np.argwhere(delta_a != 0)
        P = perimeter(binary1)
        if contours1:
            tree = cKDTree(np.vstack(contours1))
            dists, _ = tree.query(changed)
        else:
            dists = np.array([])

        mean_d = float(np.mean(dists)) if dists.size else 0.0
        max_d = float(np.max(dists)) if dists.size else 0.0
        v = var_u(binary1, binary2, contours1)
        variances.append(v)
...
# ------------------------------------------------------------
# Carga de imágenes por carpeta (sin cambios)
# ------------------------------------------------------------
def load_images_folder(folder, regex_pattern, min_index=15):
    pattern = re.compile(regex_pattern)
    files = []
...
# ------------------------------------------------------------
# Pipeline principal con cropping y guardado en carpeta fija
# ------------------------------------------------------------
def pipeline(folder,
             regex_pattern,
    ...
    # Recortar ROI [140:280, 345:495] sólo para 'resta_...'  
    for i, fname in enumerate(filenames):
        if fname.startswith("resta_"):
            images[i] = images[i][140:280, 345:495]

    # Preparar carpeta de salida común
    ...
    for k in range(len(images) - 1):
        ...
        if binary1.shape != binary2.shape:
            raise ValueError(
                f"Shapes mismatch en paso {k}: "
                f"{binary1.shape} vs {binary2.shape}"
            )

        # Guardar overlay de contornos
        base = f"step_{k:03d}_{filenames[k].rsplit('.',1)[0]}"
        save_contour_overlay(binary1, contours1, debug_folder, base)
    ...
    out_dispersion = os.path.join(output_folder, "dispersion_varu.png")
    plt.savefig(out_dispersion, dpi=300, bbox_inches='tight')
    print(f"Gráfico de dispersión guardado en: {out_dispersion}")
...
# ------------------------------------------------------------
# Ejecución en carpetas (salida común)
# ------------------------------------------------------------
if __name__ == "__main__":
    output_base = r"C:\Users\Marina\Documents\Labo 6\imagenes\test_dispersion"

    folder1 = r"C:\Users\Marina\Documents\Labo 6\imagenes\250 x 10"
    regex1 = r'resta_(\d{8}_\d{6})\.tif'
    var1 = pipeline(folder1, regex1, already_binary=False, output_folder=output_base)

    folder2 = r"C:\Users\Marina\Documents\Labo 6\imagenes\test_velocidades"
    regex2 = r".*-(\d+)\.tif$"
    var2 = pipeline(folder2, regex2, already_binary=True,  output_folder=output_base)
